...
import { average, rate, score } from "../mod.ts";
...
Deno.test("check if score() works correctly", () => {
  scoreSamples.forEach((sample) => {
    const actual = score(sample.p, sample.n);
    assertEquals(sample.e, actual);
  });
});

Deno.test("check if average() works correctly", () => {
  ratingSamples.forEach((sample) => {
    const actual = average(sample.rating);
    assertEquals(sample.expect.average, actual);
  });
});

Deno.test("check if rate() works correctly", () => {
  ratingSamples.forEach((sample) => {
    const actual = rate(sample.rating);
    assertEquals(sample.expect.score, actual);
  });
});

Deno.test("check if rate() with custom range works correctly", () => {
  customRangeSamples.forEach((sample) => {
    const actual = rate(sample.input);
    assertEquals(sample.expect, actual);
  });
});
